...
const employeeSchema = z.object({
  ...
  basic_salary: z.string().min(1, "Basic salary is required")
    .refine((v) => !isNaN(parseFloat(v)) && parseFloat(v) > 0, { message: "Must be a positive number" }),
...
export function EmployeeForm({ open, onOpenChange, employee, onSuccess }: EmployeeFormProps) {
  ...
  async function onSubmit(values: EmployeeFormValues) {
    ...
    try {
      ...
      if (!res.ok) {
        toast.error(typeof json.error === "string" ? json.error : "Failed to save employee.");
        return;
      }
      ...
      <SheetContent side="right" className="w-full sm:max-w-2xl flex flex-col p-0 overflow-hidden">
        ...
        <SheetHeader className="px-6 pt-5 pb-4 shrink-0">
          ...
          <SheetDescription className="text-slate-500 text-sm mt-1 ml-12">
            {isEditing ? "Update the employee's information below." : "Fill in the details to add a new employee."}
          </SheetDescription>
        </SheetHeader>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <Input id="full_name" placeholder="e.g. Jane Wanjiku Kamau" {...register("full_name")} />
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <Input id="id_number" placeholder="e.g. 12345678" {...register("id_number")} />
                </div>
                <div className="space-y-1.5">
                  <Label htmlFor="kra_pin">KRA PIN</Label>
                  <Input id="kra_pin" placeholder="e.g. A000000000X" {...register("kra_pin")} />
                </div>
              </div>
            </div>
            ...
            {/* Employment Details */}
            <div className="space-y-4">
              <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Employment Details</h3>

              <div className="space-y-1.5">
                <Label htmlFor="employee_number">Employee Number</Label>
                <div className="flex gap-2">
                  <Input id="employee_number" placeholder="e.g. EMP-00123 (auto-generated if blank)" className="flex-1" {...register("employee_number")} />
                  <Button type="button" variant="outline" size="icon" title="Generate" onClick={() => setValue("employee_number", generateEmployeeNumber())}>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <Input id="department" placeholder="e.g. Finance" {...register("department")} />
                </div>
                <div className="space-y-1.5">
                  <Label htmlFor="designation">Designation / Job Title</Label>
                  <Input id="designation" placeholder="e.g. Senior Accountant" {...register("designation")} />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <Select
                    ...
                    onValueChange={(v) => setValue("employment_type", v as EmployeeFormValues["employment_type"], { shouldValidate: true })}
                  >
                    <SelectTrigger>
              ...
              <div className="space-y-1.5">
                <Label htmlFor="basic_salary">Basic Salary (KES) <span className="text-red-500">*</span></Label>
                <Input id="basic_salary" type="number" min="0" step="0.01" placeholder="e.g. 50000" {...register("basic_salary")} />
            ...
            {/* NSSF / NHIF */}
            <div className="space-y-4">
              <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wider">NSSF &amp; NHIF / SHIF</h3>
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <Label htmlFor="nssf_number">NSSF Number</Label>
                  <Input id="nssf_number" placeholder="e.g. 1234567" {...register("nssf_number")} />
                </div>
                <div className="space-y-1.5">
                  <Label htmlFor="nhif_number">NHIF / SHIF Number</Label>
                  <Input id="nhif_number" placeholder="e.g. 123456789" {...register("nhif_number")} />
                </div>
              </div>
            </div>
            ...
            {/* Banking */}
            <div className="space-y-4">
              <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Banking Details</h3>
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <Label htmlFor="bank_name">Bank Name</Label>
                  <Input id="bank_name" placeholder="e.g. Equity Bank" {...register("bank_name")} />
                </div>
                <div className="space-y-1.5">
                  <Label htmlFor="bank_account">Bank Account Number</Label>
                  <Input id="bank_account" placeholder="e.g. 0123456789" {...register("bank_account")} />
                </div>
              </div>
            </div>
          </div>
